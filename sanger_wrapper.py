...
"""
    ...
    html += f"""
  <div class="footer">
    Report generated by Sanger Workflow | Python implementation of Galaxy-Workflow-Sanger.ga
  </div>
</div>
</body>
</html>
...
def main():
    parser = argparse.ArgumentParser(
        description="Sanger Workflow Wrapper - Run Galaxy Sanger workflow on custom AB1 files",
...
Examples:
  ...
  %(prog)s --auto-dir all_files/ --dry-run
    ...
    parser.add_argument("--batch", action="store_true", help="Process ALL files together (matches Galaxy behavior). Produces 1 consensus from all reads.")
    ...
    if args.batch:
        print(f"\nMode: BATCH (process all files together - matches Galaxy)")
